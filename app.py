...
def register():
    ...
    return jsonify({"message": "Registration successful", "access_token": access_token, "user": {"email": email, "name": full_name}}), 201
...
def login():
    ...
    access_token = create_access_token(identity=email)
    return jsonify({"message": "Login successful", "access_token": access_token, "user": {"email": user['email'], "name": user['full_name']}}), 200
    ...
    try:
        ...
        try:
            npk_rates = calculate_npk_rates(
                soil_n=model_payload.get('N', 0),
                soil_p=model_payload.get('P', 0),
                soil_k=model_payload.get('K', 0),
                crop_type=payload.get('crop_type', 'tomato'),
                crop_age_days=model_payload.get('growth_stage_encoded', 60)
            )
